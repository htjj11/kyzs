...
from services.第三方接口.大模型对话 import fetch_online_infomation_summary
from core.sqlLiteExec import sqlite_execute
#=======引入第三方文献专利的接口======
from services.第三方接口.石油link检索 import (
    get_articles_from_oillink,
    get_patents_from_oillink,
)
from services.第三方接口.万方检索 import (
    get_article_from_wanfang_api,
    wangfang_patent,
)
from services.第三方接口.聚合检索 import (
    get_article_from_juhe_api,
)


from services.translate_service import translate_text_api
import re
...
async def search_all_articles(
    ...
    print(f"\033[32m请求三合一文献检索: keywords={kw_list}, year={start_year}-{end_year}, page={page}\033[0m")
    loop = asyncio.get_running_loop()

    def _fetch_oilink():
        try:
            data = get_articles_from_oillink(kw_list, page - 1, int(size), user_id)
            if not data:
                return []
    ...
    def _fetch_juhe():
        try:
            data = get_article_from_juhe_api(kw_list, f'{start_year}-{end_year}', page, size)
            if not data:
                return []
...
async def search_all_patents(
    ...
    kw_list = [k.strip() for k in keywords.split(',') if k.strip()]
    kw_str = ' '.join(kw_list)
    print(f"\033[32m合并专利检索: keywords={kw_list}, year={start_year}-{end_year}, page={page}\033[0m")
    loop = asyncio.get_running_loop()

    def _fetch_oilink():
        try:
            data = get_patents_from_oillink(kw_str, page - 1, int(size))
            if not data:
                return []
